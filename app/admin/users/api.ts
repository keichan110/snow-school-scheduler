...
import type { UserWithDetails, UpdateUserRequest, UserApiResponse, UserFilters } from './types';
 ...
 * ユーザー一覧取得
 */
export async function fetchUsers(filters?: Partial<UserFilters>): Promise<UserWithDetails[]> {
  ...
  if (filters?.status && filters.status !== 'all') {
    searchParams.append('active', filters.status === 'active' ? 'true' : 'false');
  }
 ...
 * ロール表示名取得
 */
export function getRoleDisplayName(role: 'ADMIN' | 'MANAGER' | 'MEMBER'): string {
  const roleNames = {
    ADMIN: '管理者',
    MANAGER: 'マネージャー',
    MEMBER: 'メンバー',
