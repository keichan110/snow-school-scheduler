...
describe('BaseShiftMobileList', () => {
  ...
  describe('基本的なレンダリング', () => {
    it('モバイルリストコンテナが正しくレンダリングされる', () => {
      ...
      const mobileContainer = container.querySelector('.block.space-y-3.sm\\:hidden');
  ...
  describe('シフト表示', () => {
    it('シフトがある日にシフト詳細が表示される', () => {
      ...
      const mixedBadges = shiftBadges.filter((badge) => badge.getAttribute('data-count') === '1');
    ...
    it('シフトがある日に部門アイコンが正しいサイズで表示される', () => {
      ...
      const mdSizeIcons = departmentIcons.filter((icon) => icon.getAttribute('data-size') === 'md');
  ...
  describe('インタラクション', () => {
    it('日付をクリックするとonDateSelectが正しい日付で呼ばれる', () => {
      ...
      render(<BaseShiftMobileList {...defaultProps} onDateSelect={mockOnDateSelect} />);
    ...
    it('ホバー効果が適用される', () => {
      ...
      const dayItems = screen.getAllByText(/^\d+$/).map((day) => day.closest('.mobile-day-item'));
      dayItems.forEach((item) => {
        expect(item).toHaveClass('hover:-translate-y-0.5', 'hover:transform', 'hover:shadow-md');
      });
    });
  });
  ...
  describe('選択状態', () => {
    ...
    it('選択されていない日付には通常のスタイリングが適用される', () => {
      ...
      // 2月5日は選択されていない
      const day5 = screen.getByText('5').closest('.mobile-day-item');
      expect(day5).not.toHaveClass('border-blue-400', 'bg-blue-50', 'shadow-md');
    });
    ...
    it('選択状態がnullでも正常に動作する', () => {
      ...
      const dayItems = screen.getAllByText(/^\d+$/).map((day) => day.closest('.mobile-day-item'));
      const selectedItems = dayItems.filter((item) => item?.classList.contains('border-blue-400'));
