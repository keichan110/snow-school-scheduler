...
jest.mock('../BaseShiftMobileList', () => ({
  BaseShiftMobileList: jest.fn(
    ({ year, month, shiftStats, isHoliday, selectedDate, onDateSelect, variant }) => (
      <div data-testid="base-shift-mobile-list">
        ...
        <button data-testid="date-select" onClick={() => onDateSelect('2024-01-20')}>
          Select Date
        </button>
...
describe('ShiftMobileList', () => {
      ...
      shifts: [
        {
          ...
          count: 3,
          assignedInstructors: [
            { id: 1, lastName: '田中', firstName: '花子', displayName: '田中 花子' },
            { id: 2, lastName: '佐藤', firstName: '次郎', displayName: '佐藤 次郎' },
          ],
        },
        {
          type: '午後レッスン',
          ...
          count: 1,
          assignedInstructors: [
            { id: 3, lastName: '鈴木', firstName: '三郎', displayName: '鈴木 三郎' },
          ],
        },
      ],
    },
  ...
  describe('variantプロパティ', () => {
    it('admin variantを指定した場合にBaseShiftMobileListに渡される', () => {
      const { getByTestId } = render(<ShiftMobileList {...defaultProps} variant="admin" />);
    ...
    it('public variantを指定した場合にBaseShiftMobileListに渡される', () => {
      const { getByTestId } = render(<ShiftMobileList {...defaultProps} variant="public" />);
  ...
  describe('型安全性', () => {
    ...
    it('BaseShiftDisplayPropsを継承したプロパティが正しく動作する', () => {
      ...
      const { getByTestId } = render(<ShiftMobileList {...propsWithNullDate} />);
  ...
  describe('エッジケース', () => {
    ...
    it('異なる年月でも正常に動作する', () => {
      ...
      const { getByTestId } = render(<ShiftMobileList {...differentDateProps} />);
    ...
    it('複雑なシフト統計でも正常に動作する', () => {
          ...
          shifts: [
            {
              ...
              count: 1,
              assignedInstructors: [
                { id: 2, lastName: '佐藤', firstName: '花子', displayName: '佐藤 花子' },
              ],
            },
          ],
        },
      ...
      expect(getByTestId('shift-stats')).toHaveTextContent(JSON.stringify(complexShiftsStats));
    });

    it('週末・祝日の判定が正しく動作する', () => {
      const weekendHolidayProps = {
        ...defaultProps,
        isHoliday: (date: string) => ['2024-01-01', '2024-01-07', '2024-01-14'].includes(date),
      };

      const { getByTestId } = render(<ShiftMobileList {...weekendHolidayProps} />);
  ...
  describe('統合テスト', () => {
    it('管理者向けvariantと複雑なデータの組み合わせが正常に動作する', () => {
      ...
      const { getByTestId } = render(<ShiftMobileList {...adminComplexProps} />);
