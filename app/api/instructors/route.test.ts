...
const mockInstructorCreate = mockPrisma.instructor
  .create as jest.MockedFunction<typeof prisma.instructor.create>;
...
describe("POST /api/instructors", () => {
  ...
  describe("正常系", () => {
    it("必須項目のみでインストラクターが作成されること", async () => {
      ...
      mockTransaction.mockImplementation(async (callback) => {
        return callback({
          instructor: {
            create: mockInstructorCreate.mockResolvedValue(
              mockCreatedInstructor
            ),
    ...
    it("全項目を指定してインストラクターが作成されること", async () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      mockCertificationFindMany.mockResolvedValue(
        mockExistingCertifications as any
      );
      mockTransaction.mockImplementation(async (callback) => {
        return callback({
          instructor: {
          ...
          instructorCertification: {
            createMany: mockInstructorCertificationCreateMany.mockResolvedValue(
              { count: 2 }
            ),
          },
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  describe("異常系", () => {
    ...
    it("非アクティブな資格IDが指定された場合に400エラーが返されること", async () => {
      ...
      // 指定した資格が非アクティブな場合
      const mockExistingCertifications: Array<{
        id: number;
        name: string;
        isActive: boolean;
      }> = [];
      ...
      expect(mockNextResponse.json).toHaveBeenCalledWith(
        {
          ...
          error: expect.stringMatching(
            /lastName is required.*firstName is required/
          ),
        },
        { status: 400 }
      );
        ...
        mockTransaction.mockImplementation(async (callback) => {
          const mockTx = {
            instructor: {
              ...
              findUnique: jest
                .fn()
                .mockResolvedValue(mockInstructorWithCertifications),
            },
            instructorCertification: {
              createMany: jest.fn(),
            },
