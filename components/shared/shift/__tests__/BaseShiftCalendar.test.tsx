...
import { BaseShiftCalendar } from '../BaseShiftCalendar';
...
describe('BaseShiftCalendar', () => {
  ...
  describe('シフト表示', () => {
    ...
    it('複数のシフトがある日に全て表示される', () => {
      ...
      const shiftBadges = screen.getAllByTestId('shift-badge');
      const skiShifts = shiftBadges.filter((badge) => badge.getAttribute('data-count') === '2');
  ...
  describe('インタラクション', () => {
    it('日付をクリックするとonDateSelectが呼ばれる', () => {
      ...
      render(<BaseShiftCalendar {...defaultProps} onDateSelect={mockOnDateSelect} />);
  ...
  describe('エッジケース', () => {
    ...
    it('selectedDateがnullでも正常に動作する', () => {
      ...
      // 選択状態のスタイリングが適用されない
      const days = screen.getAllByText(/^\d+$/).map((day) => day.closest('.day-card'));
      const selectedDays = days.filter((day) => day?.classList.contains('border-blue-400'));
  ...
  describe('アクセシビリティ', () => {
    it('各日付セルがクリック可能である', () => {
      ...
      const dayCells = screen.getAllByText(/^\d+$/).map((day) => day.closest('.day-card'));
    ...
    it('ホバー効果が適用される', () => {
      ...
      const dayCells = screen.getAllByText(/^\d+$/).map((day) => day.closest('.day-card'));
      dayCells.forEach((cell) => {
        expect(cell).toHaveClass('hover:-translate-y-1', 'hover:transform', 'hover:shadow-xl');
      });
    });
  });
});
