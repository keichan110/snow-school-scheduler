...
import { BaseShiftModal, PublicShiftModal, AdminShiftModal } from '../BaseShiftModal';
import { DayData, DepartmentType } from '@/app/admin/shifts/types';
...
describe('BaseShiftModal', () => {
  ...
  it('should show empty state when no shifts', () => {
    ...
    expect(screen.getByText('この日はシフトの設定がありません')).toBeInTheDocument();
  });
  ...
  it('should show default footer for public variant', () => {
    render(<BaseShiftModal {...defaultProps} variant="public" dayData={mockDayDataWithShifts} />);

    expect(screen.getByRole('button', { name: '閉じる' })).toBeInTheDocument();
  });

  it('should not show default footer for admin variant', () => {
    render(<BaseShiftModal {...defaultProps} variant="admin" dayData={mockDayDataWithShifts} />);
  ...
  it('should render custom title when provided', () => {
    render(
      <BaseShiftModal {...defaultProps} dayData={mockDayDataWithShifts} title="カスタムタイトル" />
    );
  ...
  it('should return null when selectedDate is null', () => {
    const { container } = render(<BaseShiftModal {...defaultProps} selectedDate={null} />);
