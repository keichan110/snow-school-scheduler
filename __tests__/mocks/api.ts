...
export const setupApiMocks = {
  ...
  departments: {
    list: (departments: any[] = []) =>
      mockApiResponse('GET', '/api/departments', { success: true, data: departments }),

    get: (id: number, department: any) =>
      mockApiResponse('GET', `/api/departments/${id}`, { success: true, data: department }),

    create: (department: any) =>
      mockApiResponse('POST', '/api/departments', { success: true, data: department }),

    update: (id: number, department: any) =>
      mockApiResponse('PUT', `/api/departments/${id}`, { success: true, data: department }),

    delete: (id: number) =>
      mockApiResponse('DELETE', `/api/departments/${id}`, { success: true, data: null }),
  },

  // インストラクターAPI
  instructors: {
    list: (instructors: any[] = []) =>
      mockApiResponse('GET', '/api/instructors', { success: true, data: instructors }),

    get: (id: number, instructor: any) =>
      mockApiResponse('GET', `/api/instructors/${id}`, { success: true, data: instructor }),

    create: (instructor: any) =>
      mockApiResponse('POST', '/api/instructors', { success: true, data: instructor }),

    update: (id: number, instructor: any) =>
      mockApiResponse('PUT', `/api/instructors/${id}`, { success: true, data: instructor }),

    delete: (id: number) =>
      mockApiResponse('DELETE', `/api/instructors/${id}`, { success: true, data: null }),
  },

  // 資格API
  certifications: {
    list: (certifications: any[] = []) =>
      mockApiResponse('GET', '/api/certifications', { success: true, data: certifications }),

    get: (id: number, certification: any) =>
      mockApiResponse('GET', `/api/certifications/${id}`, { success: true, data: certification }),

    create: (certification: any) =>
      mockApiResponse('POST', '/api/certifications', { success: true, data: certification }),

    update: (id: number, certification: any) =>
      mockApiResponse('PUT', `/api/certifications/${id}`, { success: true, data: certification }),

    delete: (id: number) =>
      mockApiResponse('DELETE', `/api/certifications/${id}`, { success: true, data: null }),
  },
  ...
  shifts: {
    ...
    get: (id: number, shift: any) =>
      mockApiResponse('GET', `/api/shifts/${id}`, { success: true, data: shift }),

    create: (shift: any) => mockApiResponse('POST', '/api/shifts', { success: true, data: shift }),

    update: (id: number, shift: any) =>
      mockApiResponse('PUT', `/api/shifts/${id}`, { success: true, data: shift }),

    delete: (id: number) =>
      mockApiResponse('DELETE', `/api/shifts/${id}`, { success: true, data: null }),

    prepare: (prepareData: any) =>
      mockApiResponse('GET', '/api/shifts/prepare', { success: true, data: prepareData }),
  },

  // シフト種別API
  shiftTypes: {
    list: (shiftTypes: any[] = []) =>
      mockApiResponse('GET', '/api/shift-types', { success: true, data: shiftTypes }),

    get: (id: number, shiftType: any) =>
      mockApiResponse('GET', `/api/shift-types/${id}`, { success: true, data: shiftType }),

    create: (shiftType: any) =>
      mockApiResponse('POST', '/api/shift-types', { success: true, data: shiftType }),

    update: (id: number, shiftType: any) =>
      mockApiResponse('PUT', `/api/shift-types/${id}`, { success: true, data: shiftType }),

    delete: (id: number) =>
      mockApiResponse('DELETE', `/api/shift-types/${id}`, { success: true, data: null }),
  },

  // ヘルスチェック
  health: () => mockApiResponse('GET', '/api/health', { success: true, data: { status: 'ok' } }),
};
...
export const setupApiErrors = {
  ...
  // 権限エラー
  forbidden: (method: string, url: string) => mockErrorResponse(method, url, 'Forbidden', 403),

  // Not Found
  notFound: (method: string, url: string) => mockErrorResponse(method, url, 'Not Found', 404),

  // バリデーションエラー
  validation: (method: string, url: string, errors: string[] = ['Invalid input']) =>
...
export const findApiCall = (method: string, url: string) => {
  return mockApiCalls.find((call) => call.method === method.toUpperCase() && call.url === url);
};

/**
 * API呼び出し回数のアサーション
 */
export const expectApiCall = (method: string, url: string, times: number = 1) => {
