...
async function main() {
  ...
  const instructors = await Promise.all([
    ...
    prisma.instructor.create({
      data: {
        ...
        firstNameKana: "タロウ",
        status: "ACTIVE",
        notes: "スキー指導歴15年のベテラン。基礎技術指導が得意。公認指導員資格保持。",
      },
    }),
    prisma.instructor.create({
      data: {
    ...
    prisma.instructor.create({
      data: {
        ...
        firstNameKana: "ミサキ",
        status: "ACTIVE",
        notes: "スノーボード指導員として基礎〜上級まで幅広く対応。フリースタイル経験あり。",
      },
    }),
    prisma.instructor.create({
      data: {
    ...
    prisma.instructor.create({
      data: {
        ...
        firstNameKana: "ダイスケ",
        status: "ACTIVE",
        notes: "スキー・スノーボード両方の指導員資格を持つオールラウンダー。20年のキャリア。",
      },
    }),
    prisma.instructor.create({
      data: {
        ...
        firstNameKana: "マリ",
        status: "ACTIVE",
        notes: "スキーA級検定員・スノーボード準指導員の二刀流。幅広いレベルに対応可能。",
      },
    }),
    prisma.instructor.create({
      data: {
        ...
        firstNameKana: "マナブ",
        status: "ACTIVE",
        notes: "スキーB級検定員・スノーボードC級検定員として初級〜中級レッスンを担当。",
      },
    }),
    ...
    prisma.instructor.create({
      data: {
        ...
        firstNameKana: "アキラ",
        status: "RETIRED",
        notes: "昨シーズンで引退。スキー指導歴25年のレジェンド。公認指導員として多くの後輩を指導。",
      },
    }),
    prisma.instructor.create({
      data: {
  ...
  console.log(`インストラクター: ${instructors.length}名作成 (アクティブ${instructors.filter(i => i.status === 'ACTIVE').length}名、退職${instructors.filter(i => i.status === 'RETIRED').length}名、休職${instructors.filter(i => i.status === 'INACTIVE').length}名)`);
  ...
  const certificationAssignments = [
    ...
    // === スノーボード専門インストラクター ===
    // 高橋美咲 - 公認指導員 (最上位資格のみ)
    { instructorIndex: 5, certifications: [{ dept: 'snowboard', certIndex: 2 }] },
    
    // 山田健太 - A級検定員 (最上位資格のみ)
    { instructorIndex: 6, certifications: [{ dept: 'snowboard', certIndex: 5 }] },
    
    // 小林雄一 - 公認準指導員 (最上位資格のみ)
    { instructorIndex: 7, certifications: [{ dept: 'snowboard', certIndex: 3 }] },
    
    // 森さくら - 認定指導員 (最上位資格のみ)
    { instructorIndex: 8, certifications: [{ dept: 'snowboard', certIndex: 4 }] },
    
    // 吉田和也 - B級検定員 (最上位資格のみ)
    { instructorIndex: 9, certifications: [{ dept: 'snowboard', certIndex: 6 }] },
    
    // 清水麻衣 - C級検定員 (最上位資格のみ)
    { instructorIndex: 10, certifications: [{ dept: 'snowboard', certIndex: 7 }] },
    
    // === スキー・スノーボード両方の資格保持者 ===
    // 松本大輔 - スキー公認指導員 + スノーボード公認指導員
    { instructorIndex: 11, certifications: [
    ...
    { instructorIndex: 12, certifications: [
    ...
    { instructorIndex: 13, certifications: [
    ...
    { instructorIndex: 15, certifications: [{ dept: 'snowboard', certIndex: 3 }] },
  ];
  ...
  console.log(`   - 平日シフト: ${shifts.filter(s => {
    const dayOfWeek = new Date(s.date).getDay();
    return dayOfWeek !== 0 && dayOfWeek !== 6;
  }).length}件`);
  console.log(`   - 土日シフト: ${shifts.filter(s => {
    const dayOfWeek = new Date(s.date).getDay();
    return dayOfWeek === 0 || dayOfWeek === 6;
  }).length}件`);
  ...
  const skiInstructors = activeInstructors.filter((_, index) => {
    const assignment = certificationAssignments.find(a => a.instructorIndex === index);
    return assignment && assignment.certifications.some(c => c.dept === 'ski');
  });
  
  const snowboardInstructors = activeInstructors.filter((_, index) => {
    const assignment = certificationAssignments.find(a => a.instructorIndex === index);
    return assignment && assignment.certifications.some(c => c.dept === 'snowboard');
  });
  ...
  for (const shift of shifts) {
    ...
    if (shift.departmentId === skiDepartment.id) {
      if (skiInstructors.length > 0) {
        assignedInstructor = skiInstructors[skiInstructorIndex % skiInstructors.length];
        skiInstructorIndex++;
      }
    } else if (shift.departmentId === snowboardDepartment.id) {
      if (snowboardInstructors.length > 0) {
        assignedInstructor = snowboardInstructors[snowboardInstructorIndex % snowboardInstructors.length];
  ...
  console.log(`  - 資格: ${skiCertifications.length + snowboardCertifications.length}件 (スキー${skiCertifications.length}件, スノーボード${snowboardCertifications.length}件)`);
  console.log(`  - インストラクター: ${instructors.length}名`);
  console.log(`    - アクティブ: ${instructors.filter(i => i.status === 'ACTIVE').length}名`);
  console.log(`    - 退職: ${instructors.filter(i => i.status === 'RETIRED').length}名`);
  console.log(`    - 休職: ${instructors.filter(i => i.status === 'INACTIVE').length}名`);
