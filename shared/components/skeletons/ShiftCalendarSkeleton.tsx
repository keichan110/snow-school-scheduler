...
export function ShiftCalendarSkeleton({ className }: ShiftCalendarSkeletonProps) {
  return (
        ...
        {Array.from({ length: 35 }).map((_, index) => (
          ...
          >
            <div className="flex flex-col space-y-2">
              ...
              {Array.from({ length: Math.floor(Math.random() * 4) }).map((_, badgeIndex) => (
                <div key={badgeIndex} className="h-6 w-16 rounded-full bg-muted" />
              ))}
            </div>
          </div>
        ))}
